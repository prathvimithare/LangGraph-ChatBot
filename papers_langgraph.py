...
vector_client = VectorStore(collection_name="papers_chunks", persist_directory="ChromaDB", ollama_host=Config.OLLAMA_HOST)
...
def retriever_tool(query: str) -> str:
    """
    This tool searches and returns relevant publications from the knowledge base.
    
    Each publication includes the **title** and the **abstract**. 
    This helps users quickly understand the content of each paper.
    
    The tool also provides metadata for proper citation, including:
    - title of the paper
    - id
    
    Use this tool to retrieve authoritative references for user questions.
    """
...
system_prompt = """
You are an intelligent AI assistant specialized in answering questions about scientific publications stored in the knowledge base. 
...
- Base your answers solely on information retrieved from the knowledge base.  
- Always cite the **title** and **publication ID (id)** when referencing a paper.  
- If the knowledge base does not contain sufficient information, state this clearly.  
